...
// transportConn 只适配 wsnet 已有所有权和队列语义，不增加第二条发送队列。
type transportConn struct{ conn *wsnet.Conn }
...
// runtimeHandler 把并发 wsnet 回调收束到一个公共 network Runtime。
type runtimeHandler struct {
	runtime *core.Runtime
	opened  func(*core.Session, *wsnet.Conn)
	closed  func(*core.Session, *wsnet.Conn, error)

	mu       sync.RWMutex
	sessions map[*wsnet.Conn]*core.Session
}

func newRuntimeHandler(runtime *core.Runtime) *runtimeHandler {
	return &runtimeHandler{
		runtime:  runtime,
		sessions: make(map[*wsnet.Conn]*core.Session),
	}
}

func (handler *runtimeHandler) OnOpen(conn *wsnet.Conn) {
...
func (handler *runtimeHandler) OnWritableChanged(conn *wsnet.Conn, writable bool) {
	handler.mu.RLock()
	session := handler.sessions[conn]
	handler.mu.RUnlock()
	if session != nil {
...
func (handler *runtimeHandler) OnClose(conn *wsnet.Conn, cause error) {
	handler.mu.Lock()
	session := handler.sessions[conn]
...
var (
	_ wsnet.Handler         = (*runtimeHandler)(nil)
	_ wsnet.WritableHandler = (*runtimeHandler)(nil)
