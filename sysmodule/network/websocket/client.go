...
// Client 是由 Service 生命周期托管的单连接 WebSocket Client Module。
type Client struct {
	service.Module
...
// NewClient 校验并创建尚未绑定 Service 的 WebSocket Client Module。
func NewClient(url string, options ClientOptions) (*Client, error) {
	if err := validateClientOptions(url, options); err != nil {
		return nil, err
	}
	options.Dial = freezeDialOptions(options.Dial)
	return &Client{
		url:     url,
		options: options,
		state:   network.ClientStateSnapshot{State: network.ClientStopped},
	}, nil
}
...
func (client *Client) OnStart(ctx context.Context) error {
	...
	conn, dialErr := wsnet.Dial(
		clientCtx,
		client.url,
		internalDialOptions(runtime, client.options.Dial),
		handler,
	)
	if dialErr != nil && !client.options.Reconnect.Enabled {
		...
		client.setStateDirect(ctx, network.ClientStateSnapshot{
			State: network.ClientStopped, Attempt: 1, LastError: dialErr,
		})
		return dialErr
	}
	client.mu.Lock()
...
func (client *Client) onOpened(session *core.Session, conn *wsnet.Conn) {
	client.mu.Lock()
...
func (client *Client) onClosed(session *core.Session, conn *wsnet.Conn, cause error) {
	...
	for {
		...
		if !client.options.Reconnect.Enabled {
			client.updateState(network.ClientStateSnapshot{
				State: network.ClientStopped, LastError: cause,
			})
			return
		}
		connected := false
		for attempt := 1; attempt <= client.options.Reconnect.MaxAttempts; attempt++ {
			...
			client.updateState(network.ClientStateSnapshot{
				State: network.ClientReconnecting, Attempt: attempt,
				NextDelay: delay, LastError: cause,
			})
			...
			conn, err := wsnet.Dial(
				client.ctx,
				client.url,
				internalDialOptions(client.runtime, client.options.Dial),
				client.handler,
			)
			if err != nil {
				cause = err
	...
	defer func() {
		if value := recover(); value != nil {
			client.Logger().ErrorStack(
				"WebSocket Client StateChange panic",
