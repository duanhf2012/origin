...
// Client 是由 Service 生命周期托管的单 Session KCP Client Module。
type Client struct {
	service.Module
...
// NewClient 校验并创建尚未绑定 Service 的 KCP Client Module。
...
// OnStart 建立 Runtime，并创建一次初始 KCP Session。
...
// OnStop 取消创建/退避，关闭活动 Session，等待唯一重连 Worker，再交付最终关闭事件。
...
func (client *Client) onOpened(session *core.Session, conn *kcpnet.Conn) {
	client.mu.Lock()
...
func (client *Client) onClosed(session *core.Session, conn *kcpnet.Conn, cause error) {
