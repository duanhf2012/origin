...
type NoDelayConfig struct {
	...
	Interval originconfig.Duration `json:"interval"`
	// FastResend 是累计多少次跨越 ACK 后触发快速重传；零表示关闭。
	FastResend int `json:"fast_resend"`
	// DisableCongestionControl 关闭 KCP 拥塞控制，以带宽换取更低时延。
	DisableCongestionControl bool `json:"disable_congestion_control"`
}
...
type ServerConfig struct {
	...
	NoDelay NoDelayConfig `json:"no_delay"`
	...
	SocketReadBuffer originconfig.ByteSize `json:"socket_read_buffer"`
	// SocketWriteBuffer 设置 UDP 发送缓冲；0B 保留操作系统默认值。
	SocketWriteBuffer originconfig.ByteSize `json:"socket_write_buffer"`
