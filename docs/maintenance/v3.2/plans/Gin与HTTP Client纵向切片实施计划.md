...
- `sysmodule/ginmodule` 的可嵌入 HTTP Module、RouterGroup、ServerConfig、ServerOptions、生命周期、安全边界
  和固定统计；
- `sysmodule/httpclient` 的可复用 Client、默认 Transport、流式请求和有界响应读取；
- 一个匿名嵌入 `ginmodule.Module`、集中定义路由和业务 Handler、并持有 HTTP Client 的最小 Example；
