...
- [ ] **Step 1: 写 `GetNode` 和 Node 时间语义的失败测试**
...
- [ ] **Step 2: 运行测试并观察旧实现错误使用 `timerEngine.Now()`**
...
- [ ] **Step 1: 写前进、后退和跨 Service 重排失败测试**
...
- [ ] **Step 2: 运行测试并确认旧 Deadline 没有随偏移更新**
...
- [ ] **Step 3: 实现 Scheduler 重排和 Node 广播**

  `RebaseTimers` 在 Scheduler 锁内只处理 Scheduled：取消旧 Deadline、删除 Binding、增加代次、
  按 `fireAt - runtime.Now()` 重新登记并恢复 Binding。Node Set/Add 在线性化更新偏移后按稳定
