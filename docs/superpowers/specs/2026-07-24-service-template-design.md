...
模板名称是否作为额外元数据发布，以及 `allow_discovery` 是否支持按模板名称筛选，将在服务发现筛选设计中单独确定。本文不让 `services` 字段同时匹配实际名称和模板名称，以免产生隐式歧义。
...
1. `allow_discovery` 如何显式筛选同一模板创建的全部实例；
2. 模板构造器的注册 API；
3. 模板实例的配置传递与覆盖规则；
4. 模板实例与 Node 启停顺序的关系；
5. 模板实例的单目标 RPC 选择和广播规则。
