...
契约文件中的 `//go:generate go run ...` 也属于这一种方式。契约位于普通目录时，可以在项目
根目录执行 `go generate ./...`。Go 的 `./...` 会跳过名称以 `_`、`.` 开头的目录和
`testdata`；这类目录必须显式指定。本仓教程合约位于 `_support`，因此应使用示例自带脚本，
或执行 `go generate ./examples/_support/tutorialrpc`。生成器版本由项目依赖约束，不会因
开发者本机的全局工具版本不同而产生差异。
...
```bash
# 1. 运行当前包声明的生成命令；普通目录中的契约也可统一使用 go generate ./...。
...
直接运行示例：[Async、Notify 与 Broadcast](../../../../examples/06-rpc-basics/02-async-and-notify)。

`Async` 只表示请求已提交，结果在回调中处理；`Notify` 不等待也不返回业务结果，适合可容忍单向语义的通知。

## 深入一点：何时选择哪种调用

- `Await`：Service Task 必须依赖返回值时使用；传入当前任务 Context。
- `Call`：普通 goroutine 需要在当前调用栈取得结果时使用；不要在 Service Task 中调用。
- `Async`：需要继续处理其他工作，结果可在回调中消费时使用。
- `Notify`：无需返回值，可接受远端业务错误仅在目标诊断侧记录时使用。
- `Broadcast`：向当前范围内全部匹配目标投递通知，并接收部分失败结果。

运行期的 Service 可以调用自身 RPC。在 Service Task 中使用 `AwaitXxx`、`AsyncXxx`
或 `NotifyXxx`：`AwaitXxx` 会暂时释放执行槽，让自调请求进入同一 FIFO；
`AsyncXxx` 和 `NotifyXxx` 提交后必须让当前任务返回，后续任务才能执行。
`CallXxx` 只适合不持有 Service 执行槽的普通 goroutine；在 Service Task 中自调
会占住唯一执行槽，只能等 Deadline 退出。

生命周期回调可以使用 Await，但同 Node 的 Service 只会在全部 `OnStart` 成功后统一进入
Running，停止阶段也会关闭新 RPC 准入；`OnStart`/`OnStop` 不要调用同 Node 业务 RPC。
...
同 Node 本地调用会保留 Context Value，但任意 Go Value 不会通过 TCP/NATS 自动序列化。
其他 Node 必需读取的数据应明确放进 RPC 参数；Context 继续用于本次调用的取消和 Deadline。

完整的可选 Context、Call 和生命周期边界见
[RPC Context 与调用规则](../../../maintenance/v3.1/guides/README.md)。
