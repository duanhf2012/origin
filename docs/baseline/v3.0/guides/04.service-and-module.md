...
只有职责具有清晰边界，或需要独立管理 Timer、事件监听器和启停资源时才拆成 Module。仅为了
减少单个文件行数而拆分，会增加生命周期顺序和归属理解成本，通常保留为普通 Go 类型或方法
更简单。Module 树在 `OnInit` 结束后冻结；单个 Service 最多登记 4096 个 Module，包含根节点
在内最多嵌套 64 层，超过限制会使启动失败。
...
更适合跨对象的冷路径状态查询见 [10：Admin、Diagnostics 与 pprof](../../../maintenance/v3.1/guides/10.admin-diagnostics-and-pprof.md) 的
