...
同步监听器也可以直接等待数据库、Redis、文件或其他 I/O：

```go
func (s *PlayerService) onLoadedWithAwait(ctx context.Context, raw service.Event) error {
    loaded := raw.(PlayerLoaded)
    var extra PlayerExtra
    if err := s.Await(ctx, func(waitCtx context.Context) error {
        var loadErr error
        // 等待期间已释放 Service 执行权，只写局部结果。
        extra, loadErr = s.loadExtraData(waitCtx, loaded.PlayerID)
        return loadErr
    }); err != nil {
        return err
    }
    // Await 恢复后已重新取得 Service 执行权，再更新成员状态。
    s.playerExtra[loaded.PlayerID] = extra
    return nil
}
```

这个函数需要在 `OnInit` 注册后才会成为监听器：

```go
_ = s.SubscribeEvent(playerLoadedEvent, s.onLoadedWithAwait)
```

生成的 Await RPC 使用同一套调度语义，不需要改用其他接口：

```go
func (s *PlayerService) onLoadedWithRPC(ctx context.Context, raw service.Event) error {
    loaded := raw.(PlayerLoaded)
    player, err := s.players.AwaitGetPlayer(ctx, loaded.PlayerID)
    if err != nil {
        return err
    }
    // RPC Await 已恢复 Service 执行权。
    s.currentPlayer = player
    return nil
}
```

此时“同步”保证两件事：调用方等到所有监听器完成后才从
`NotifyEventSync` 返回；同一事件的监听器仍按注册顺序调用。如果前一个监听器
Await，它恢复并返回后才会调用下一个监听器。

但 Await 期间会释放 Service 执行权，所以同 Service 的其他已就绪任务可以插入执行。
监听器恢复后，Await 前观察的 Service 状态可能已改变；应按本章前面的 Await
数据规则使用局部结果，必要时重新校验业务状态。

只有当业务不希望调用方等待监听器完成时，才改用 `NotifyEventAsync`；它只给调用方
“已入队”的保证。
