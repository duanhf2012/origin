...
运行：[examples/10-diagnostics-and-pprof/01-diagnostics-snapshot](../../../../examples/10-admin-diagnostics-and-pprof/03-diagnostics-snapshot/README.md)。
...
内部顺序。完整可运行代码见：[诊断快照示例](../../../../examples/10-admin-diagnostics-and-pprof/03-diagnostics-snapshot/README.md)。
...
运行：[examples/10-diagnostics-and-pprof/02-diagnostics-server](../../../../examples/10-admin-diagnostics-and-pprof/04-admin-diagnostics/README.md)。启动参数为：
...
运行：[examples/10-diagnostics-and-pprof/03-pprof-toggle](../../../../examples/10-admin-diagnostics-and-pprof/05-pprof-toggle/README.md)。
...
运行：[examples/10-diagnostics-and-pprof/04-metrics-adapter](../../../../examples/10-admin-diagnostics-and-pprof/06-metrics-adapter/README.md)。示例把 `diagnostics.Source` 转换为业务自定义指标，不直接绑定 Prometheus。业务可在适配层决定采集周期、缓存、指标名、标签上限和 Prometheus/OpenTelemetry 导出方式，Origin 的 RPC 热路径不会因此增加动态标签或逐请求 Histogram。
