...
下表汇总业务项目实际会接触到的 `Application` 方法。除 `Start` 外，方法均可在持有具体 `*application.Application` 的程序入口、管理 Service 或受控后台任务中调用；普通业务 Service 通过 `s.Application()` 只能得到受限诊断外观，详见 [10：Diagnostics 与 pprof](./10.diagnostics-and-pprof.md)。
...
| `State`、`Node(id)`、`Nodes`、`Logger` | 管理/观测代码；`Nodes` 返回独立 Slice 快照 | [10：Diagnostics 与 pprof](./10.diagnostics-and-pprof.md) |
| `Retire(ctx)`、`Resume(ctx)` | 管理 Service 或发布编排 | [09：Retire 与 Resume](./09.retire-and-resume.md) |
| `Diagnostics`、诊断 HTTP、pprof 方法 | 受控观测和按需诊断 | [10：Diagnostics 与 pprof](./10.diagnostics-and-pprof.md) |
