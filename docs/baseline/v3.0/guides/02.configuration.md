...
配置解码优先使用标准 `json` Tag；没有 Tag 时使用 Go 字段原名精确匹配，不会自动转换
大小写或下划线。JSON 与 YAML 使用完全相同的规则：

```go
type serviceConfig struct {
    // 无 Tag：配置字段必须写成 RefreshInterval。
    RefreshInterval originconfig.Duration
    // 显式 Tag：JSON/YAML 配置字段都写成 max_players。
...
项目希望配置统一使用 `snake_case` 时，应像示例一样显式声明 `json` Tag。`yaml` Tag 和
自定义 `config` Tag 不参与 Origin 配置解码；字段拼写必须与最终名称精确一致。
