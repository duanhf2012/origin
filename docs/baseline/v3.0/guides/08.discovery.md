...
`discovery.type: origin`。它复用 Application 的 RPC 传输：下面以 TCP 为例，NATS 时只需将
顶层 `rpc` 改成 NATS 配置，Node 内不再写 `rpc`：
...
如果分区一的网关还需要发现分区二的服务，增加 `watch_networks`：
...
`watch_networks` 只增加读取范围，不会把当前 Node 注册到其他网络。`local_network` 与 `watch_networks` 去重后最多允许 64 个网络。
