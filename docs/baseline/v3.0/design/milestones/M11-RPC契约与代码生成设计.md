...
3. 客户端、静态编解码入口、Dispatcher 和冷启动描述符均由 `origingen` 在契约包生成；
   业务 Service 包不生成代码；
...
- 契约包生成冷启动描述符，Node 按 Service 模板名自动关联业务实现；
- 静态 Dispatcher，不生成业务 Service 适配方法；
...
4. 按契约名计算默认 Service 模板名和冷启动描述符；
5. 全局计算并检查 ContractID、MethodID；
6. 在渲染任何文件前完成全部签名、类型图、名称、Schema、Codec 计划和碰撞校验；
7. 所有校验通过后才在内存中生成完整文件；
8. 普通模式使用临时文件和同目录原子替换提交结果；
9. `--check` 模式逐字节比较应生成内容与工作树，发现缺失、过期或多余生成文件时返回
    非零状态，但不得修改文件。
...
每个包含 RPC 契约的源文件生成一个同名 RPC 文件：

```text
player_service.go -> player_service.rpc.gen.go
```
...
- 业务不能手工修改生成文件；
- 契约和业务实现应分包放置；生成客户端、静态 Dispatcher 和冷启动描述符都留在契约包，
  业务实现包不生成文件；
- 旧版 `zz_origin_rpc.gen.go` 仅在带完整生成标记时由生成器自动删除并迁移；
- CI 必须能够重新生成并检查工作树无差异。

契约与实现可以位于不同包或不同 Go Module；业务 Module 只需依赖已经生成的契约包。
自动绑定不扫描实现源码，而是在 Node 冷启动时使用 Go 接口断言校验实际实例。
...
`origingen` 只扫描带 `//origin:rpc` 的契约包。每个契约生成客户端、静态 Dispatcher 和
一份不可变冷启动描述符。描述符包含默认 Service 模板名、契约全名、ContractID、指纹和
类型断言式 Dispatcher 工厂；生成包通过 `init` 登记描述符。

生成器不扫描业务 Service，不需要知道实现散落在哪些业务文件，也不会在业务目录生成
`*.gen.go`。业务实现只建议保留普通 Go 编译期断言：

```go
var _ playerapi.PlayerService = (*PlayerService)(nil)
```

断言只用于尽早发现漏实现的方法，不参与运行时查找或 RPC 热路径。

### 10.2 模板名与实际名

默认关联名由契约名确定：`PlayerService`、`PlayerRPC` 和 `PlayerServiceRPC` 都对应模板名
`PlayerService`。Node 装配使用 `ServiceBinding.Template` 查找描述符，而不是使用改名后的
`ServiceBinding.Name`。

配置 `player-1:PlayerService` 和 `player-2:PlayerService` 因此都复用同一契约与 Dispatcher；
实际名 `player-1`、`player-2` 仍用于配置覆盖、发现、本地查询和 RPC 路由。框架不提供
`SetName`，避免代码与配置形成两个名称来源。调用改名实例时使用生成的 `Bind...To`。

同一个契约允许多个 Service 实例或实现类型；同一进程内两个不同契约若争用同一默认模板
名，描述符注册冲突并在 Node 冷启动时明确失败。没有匹配描述符的普通 Service 正常运行。
...
3. 使用绑定的 `Template` 在生成描述符注册表中查找契约；
4. 描述符工厂通过静态接口断言校验业务实现，并创建 Dispatcher；
...
描述符注册表只在进程初始化和 Node 冷启动装配时使用。成功注册后，调用仍直接进入每个
Node 私有 Runtime 保存的静态 Dispatcher；热路径不查全局表、不反射，也不增加编解码或
网络开销。
...
| 3 | RPC 契约可见性、泛型和跨包实现桥接 | RPC 接口和方法要求导出、首版禁止泛型契约；契约包生成 `New<Contract>Dispatcher(impl Contract)` 和冷启动描述符；Node 按模板名自动关联，Service 包不生成文件 | 已确认，最终优化 |
...
| 11 | 是否公开只读 Descriptor | 仅公开供生成代码登记的最小 `GeneratedContractDescriptor`；它不作为业务查询或监控 API，只参与冷启动自动装配 | 已确认，最终优化 |
