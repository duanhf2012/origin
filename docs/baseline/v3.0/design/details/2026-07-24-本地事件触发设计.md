...
8. 所有事件监听者由框架提供 Origin Task Context；异步监听者可以把取消、Deadline 和当前
   Service 执行能力继续传给合法的 Await，同步监听者禁止 Await。

## 3. 与 v2 的关系

v2 的 `NotifyEvent` 不是同步函数调用。它把事件投递到事件通道，监听者在所属 Service 的后续事件处理中收到回调。

v3 保留这一能力，但不继续使用容易混淆的单一名称。事件和公共接口采用：
...
func (s *Service) SubscribeEvent(
    eventID EventID,
    handler EventHandler,
) error

func (s *Service) NotifyEventAsync(event Event) error
func (s *Service) NotifyEventSync(
    ctx context.Context,
    event Event,
) error
```

`SubscribeEvent`、`NotifyEventAsync` 和 `NotifyEventSync` 都是 `service.Service` 的成员方法，
业务使用嵌入的 `service.Service` 直接调用，不再把 Service 指针作为普通函数参数传入。
`service.Module` 提供同名成员方法并委托给所属 Service 的同一事件总线。

Go 不支持带独立类型参数的方法，因此首版不为这三个成员方法增加泛型包装层。监听者按 `EventID`
订阅并接收 `Event`，业务在监听函数边界完成一次明确的类型断言；将来若真实项目证明有必要，
可由生成器在业务包中增加类型安全的薄辅助函数，不改变底层成员接口。
...
- 异步监听者需要调用 Await 时使用 `m.Await(ctx, fn)` 或 `s.Await(ctx, fn)`；
- 同步监听者禁止 Await，原因和示例见第 6.3 节；
...
### 6.3 为什么同步监听者禁止 Await

假设玩家当前等级是 10，当前 Service Task 同步发布等级变化：

```go
func (s *PlayerService) levelChanged(ctx context.Context) error {
    s.level = 10
    return s.NotifyEventSync(ctx, PlayerLevelChangedEvent{Level: 10})
}
```
...
func (m *RankModule) onLevelChanged(
    ...
    changed := event.(PlayerLevelChangedEvent)

    // Await 会释放当前 Service 执行权。
    if err := m.Await(ctx, m.loadRank); err != nil {
        return err
    }

    // 等待期间，另一个 Service Task 可能已经把玩家升到 11 级。
    // changed.Level 仍是旧快照 10，继续写状态前必须重新校验。
    return m.applyLevel(changed.Level)
}
```

此时 `NotifyEventSync` 虽然仍会等所有监听者完成才返回，但执行链中间已经允许其他 Service Task
运行，“同步”不再等于不可交错的当前调用栈，容易让业务误判。因此 M21 已确认：

- 同步监听者只处理当前内存状态，不允许 Await；
- 需要 RPC、数据库、Redis 或文件 I/O 的监听逻辑改用 `NotifyEventAsync`；
- 异步监听者是普通 Service Task，可以 Await，并在恢复后按既有规则重新校验状态；
- 同步监听者可以继续嵌套发布同步事件，但框架把嵌套深度限制为 64，超过后返回错误，避免事件环耗尽调用栈。
...
```go
func (m *PlayerModule) levelChanged(
    ctx context.Context,
    playerID string,
    level int32,
) error {
    return m.NotifyEventSync(ctx, PlayerLevelChangedEvent{
        PlayerID: playerID,
        Level:    level,
    })
}
...
单个监听者返回 error 或发生 panic 时，框架在该监听者边界隔离错误并继续按注册顺序调用
后续监听者。panic 处理必须：
...
同步触发在全部监听者完成后，按注册顺序返回聚合错误；异步触发没有直接调用方接收监听结果，
因此记录一次聚合摘要、监听者维度指标和 panic 本地堆栈，不因单个观察者故障阻断其他观察者。
...
9. 每个 Service 最多注册 4096 个非零 `uint32 EventID` 和 65536 个监听者，按需增长且不按上限预分配。
10. 同一 Service 内相同 EventID 必须绑定相同 payload 类型；反射最多在 `OnInit` 冲突校验时使用，派发热路径不反射查类型。
11. 一次异步事件只占一个 M9 `MaxTasks` Ready 项；全部监听者在该任务内借用同一逻辑 payload。
12. 异步投递成功后生产者不能继续修改 payload 引用的可变数据；框架不做反射深复制。
...
18. 单个监听者 error/panic 后继续执行后续监听者，并按同步/异步规则聚合；
19. 同步监听者调用 Await 被稳定拒绝；
20. 同步事件嵌套深度 64 可用，超过 64 返回统一错误且不破坏调用栈；
21. EventID、监听者容量、payload 类型冲突和按需分配边界；
22. 异步事件只占一个 Ready 项，生产者修改已投递可变 payload 的行为不由框架兜底；
23. 高频同步与异步触发的延迟、吞吐和分配基准。
...
- `SubscribeEvent`、`NotifyEventSync` 和 `NotifyEventAsync` 均为 Service 成员方法；
- Module 提供同名成员方法，并委托给所属 Service；
- 不提供语义不明确的 `NotifyEvent`；
- 不通过 `EventMode` 参数在运行期选择同步或异步；
- 异步事件进入 Service 普通 FIFO Ready 队列；
- 同步事件直接按注册顺序调用监听者；
- 监听者 error/panic 隔离后继续执行后续监听者；同步返回聚合错误，异步记录聚合摘要和指标；
- 同步监听者禁止 Await，需要等待 I/O 时改用异步事件；
- 同步事件允许嵌套，但最大深度为 64；
- 同步事件只能由持有所属 Service 执行权的调用方触发；
- 外部 goroutine 只能异步投递；
- Module 停止时自动清理其事件订阅；
- 事件派发不创建 Module 独立调度器或 goroutine；
- EventID 为非零 uint32，每 Service 最多 4096 个 EventID 和 65536 个监听者，不预分配；
- 一次异步事件只占一个普通 Ready 项，监听者借用同一逻辑 payload，框架不深复制。
