...
`service.Module` 为三个生命周期回调提供默认空实现。业务只覆盖需要的回调：
...
Module 直接委托所属 Service 的 Scheduler、Timer、Await、RPC、配置和共享业务资源，因此
Service 是 Module 的生命周期父级。`Service.OnStart` 先建立 Service 级资源，Module 随后按
静态树顺序启动；`Service.OnStart` 不得依赖尚未启动的 Module 执行业务操作。全部 Module
启动成功前，Service 仍处于 `Starting`，不会提前对外可用。

父先子后的依赖约束是：

- Module 可以依赖已经完成 `OnStart` 的所属 Service；
...
Module 在 `OnStop` 中仍可使用所属 Service 尚未释放的 Scheduler、Await、RPC、配置和共享
资源。全部 Module 都完成停止尝试后才调用 `Service.OnStop`，使 Service 可以汇总 Module 已经
提交的最终结果并关闭 Service 级共享资源；`Service.OnStop` 不得再次要求已停止 Module 执行
业务操作。
...
- 跨 Module 的收尾应由各 Module 在自己的 `OnStop` 中提交结果，`Service.OnStop` 只汇总
  已提交结果并释放 Service 级共享资源。
...
func (m *PlayerModule) OnStart(ctx context.Context) error {
    m.saveTimer = m.NewTicker(time.Minute, m.save)
...
`Await` 与 `RunSafe` 的边界不同：Await 用于 RPC、数据库、Redis 等等待，普通业务 Task 中会
释放 Service 执行权并在完成后按 FIFO 恢复；RunSafe 只在当前 goroutine 同步执行一个已经与
Service 可变状态隔离的 Job，把该 Job 的 panic 转成错误，不释放执行权也不参与调度。
长期 Worker 使用 GoSafe 保护最外层只能避免进程退出；如果希望单个 Job panic 后 Worker 继续
处理下一项，还要在循环内对每个 Job 使用 RunSafe。普通 I/O 等待不需要套 RunSafe。
...
Module 与 Service 共享 Node 的 TimerEngine、RPC Runtime 和 ServiceScheduler。资源隔离只
表示所有权和批量清理边界，不表示框架为每个 Module 自动创建独立引擎、队列或业务
goroutine。业务 Module 可以按需要创建自己持有的 AOI 等固定 Worker，但 Worker 不获得第二
个 Service 执行槽；并发计算结果通过所属 Service 的 `DispatchAsync` 交回，Module 在
`OnStop` 中负责 cancel 和 wait。
...
1. 立即停止当前 Service 的 Module 树构建，不再初始化尚未进入 `OnInit` 的兄弟 Module
   和子 Module；
2. 当前 Service 的初始化作用域记录首个 Module 错误；即使业务忽略 `AddModule` 返回值，
   后续 `AddModule` 也会拒绝执行，不能继续拼出部分有效的 Module 树；
3. 当前 Module 及其未完成的子树不进入有效 Module 树；
4. 框架清理已经注入的父子引用、事件订阅和仅用于静态装配的内部资源；
5. 不调用任何 Module 的 `OnStop`，因为 `OnInit` 不允许取得需要通过业务回调释放的外部
   资源；
6. 当前 Service 的 `OnInit` 最终按失败处理，但 Node 继续执行后续 Service 的 `OnInit`，
   最后按 Service 配置顺序聚合错误。
...
2. 只要当前 Module 的 `OnStart` 已经被调用，无论成功或失败，框架都把它加入实际启动
   记录，并保证调用一次 `OnStop`；
3. 当前 Module 必须在 `OnStart` 内通过局部 `defer` 清理尚未交给 Module 持有的临时
   资源；已经交给 Module 持有的资源由 `OnStop` 清理；
4. 当前失败 Module 和此前进入过 `OnStart` 的 Module 按实际启动记录严格逆序执行
   `OnStop`；
5. 全部已进入的 Module 都停止后调用一次 `Service.OnStop`；
6. Service 不对外可用。
...
如果 `Service.OnStart` 失败：

- 失败 Service 自身已经真正进入 `OnStart`，因此框架调用一次 `Service.OnStop`；
- Service 通过局部 `defer` 清理 `OnStart` 中尚未交给自身持有的临时资源；
- 任何 Module 的 `OnStart` 都不会执行，也不会调用 `Module.OnStop`；
- Service 不进入 `Running`。
...
同步、异步和订阅都直接使用 Module 成员接口；Module 内部委托给所属 Service 的同一事件总线，
业务不需要把 Service 指针作为参数传入：

```go
func (m *Module) SubscribeEvent(eventID EventID, handler EventHandler) error
func (m *Module) NotifyEventAsync(event Event) error
func (m *Module) NotifyEventSync(ctx context.Context, event Event) error
```
...
1. 框架不为每个 Module 自动增加独立 goroutine、Mailbox、Runner 或调度锁；业务明确需要
   的固定 Worker 由 Module 自己持有，优先通过所属 Service 的 `GoSafe` 启动并在停止时
   cancel/wait；Worker 循环需要隔离单个 Job panic 时使用所属 Service 的 `RunSafe`。
...
7. 每个 Service 最多包含 4096 个 Module，Module 树深最多 64；内部集合按需增长，不按上限预分配。
...
3. 同级 Module 按添加顺序启动；
4. 树形 Module 按严格逆序停止；
5. `Service.OnStart` 在全部 Module 启动前执行；
6. `Service.OnStop` 在全部 Module 停止后执行；
7. Module `OnInit` 失败时停止当前 Service 的 Module 树构建、完整解除无效子树，但继续
   后续 Service 的 `OnInit` 并最终聚合错误；
8. Module `OnStart` 失败时，失败 Module 自身和此前进入过 `OnStart` 的 Module 都恰好
   执行一次 `OnStop`，并按实际进入顺序严格反序回滚；
...
19. 1/100/1000/4096 Module 的初始化、启动、停止、回滚和分配基准；
20. 第 64 层 Module 可用，第 65 层被稳定拒绝且已构建资源完整回滚。
...
- 业务 Module 嵌入 `service.Module`，默认回调为空实现；
...
- 启动采用父先子后、同级按添加顺序；
- 停止采用启动顺序的严格逆序；
- `Service.OnStart` 在所有 Module 启动前执行；
- `Service.OnStop` 在所有 Module 停止后执行；
- 任一 Module 启动失败都会使 Service 启动失败；
- Module `OnInit` 失败立即终止当前 Service 的 Module 树构建，但 Node 继续检查其他
  Service 的 `OnInit`；
- 只要 Module 的 `OnStart` 已被调用，即使返回错误也调用一次 `OnStop`；
- Module 直接组合 Timer、Await、异步任务和本地事件接口；
- Module 通过所属 `IService` 读取 Service 业务配置；
- 所有能力委托给所属 Service，不创建 Module 独立调度器；
- 框架不自动给 Module 创建 goroutine；业务 Module 可以持有固定 Worker，但不并发访问
  Service 可变状态，结果通过 `DispatchAsync` 返回唯一执行槽，并自行负责 cancel/wait；
- Module Worker 使用 `GoSafe` 保护 goroutine 顶层，使用 `RunSafe` 隔离单个 Job panic；
  两者都不创建 Module 独立执行槽；
- 每个 Module 具有内部资源作用域；
- 每个 Service 最多 4096 个 Module、树深最多 64，按需增长且不预分配上限容量；
