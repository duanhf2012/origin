...
- Service 查询；
- RPC 候选路由构建；快照保留 Running/Retired，自动选择默认只取 Running，显式
  `IncludeRetired()` 后取 Running 与 Retired，精确目标始终允许 Retired；
- Broadcast 目标 Node 计算；其自动范围使用同一 Retired 规则；
- TCP 连接需求计算；
...
状态拒绝入站或移出精确路由；M19/M20 自动范围默认排除 Retired，显式
`IncludeRetired()` 后纳入，业务仍可在目标 Service 内根据状态自行处理。
...
匹配 Service 进入退休状态不等于消失。实例仍在可见快照和精确 RPC 目标中，TCP 连接
需求不变。M19 的 `ToService` 自动选择会摘除 Retired；状态发布、精确调用和连接保留不
冒充 Lost。
...
Service 一定会接受业务请求。Retired 继续可见且可精确调用；M19/M20 自动范围默认排除，
显式 `IncludeRetired()` 后纳入。
...
退休只改变公开运行状态，不冒充 `Lost` 或 Transport 断线；它不改变精确路由和入站准入，
但 M19 自动路由会停止向它分配新流量。
...
29. Service 退休后仍在可见快照和精确 RPC 目标中，但不进入 `ToService` 自动候选；
...
- 退休实例保持可见、连接和精确 RPC 路由能力，自动路由只选 Running；
...
28. `Retired` 只作为可观察状态发布，框架不自动拒绝精确 RPC/Notify/Broadcast；M19/M20
    自动范围默认排除 Retired，并由 `IncludeRetired()` 显式纳入；
