...
M6 实现一个只面向 Origin 框架内部的 Core NATS 基础库，为 M11 NATS RPC、后续 NATS
...
调度。M11 再把 M9 RPC Runtime 与 M6 组合起来。
...
M10/M11 的 RPC 使用方在需要时定义最小适配接口。M6 不为了外观统一而让 TCP 假装拥有
Subject，也不让 NATS 假装拥有逐 Node TCP Connection；但 M10/M11 必须把两者接入同一
...
M6 只实现 Options 和校验，不在本里程碑把该结构接入完整 Node 配置；M7/M11 接入时再把
...
2. M11 可以把 `bufferpool.Buffer.Bytes()` 传给 M6，并在 Publish 返回后释放 Buffer；
...
- M11 若需要把消息转入异步 Service 调度队列，应在 NATS 回调中完成必要的协议校验，
  再把明确拥有的数据对象投递给调度器；
...
- M11 可以用 BufferPool 构建统一 RPC 线协议，Publish 返回后立即归还 Buffer；
- M11 的 NATS 回调必须快速校验和解码，不能无界持有 Data；
- RPC Runtime 在回调返回后不得继续引用原始 Data；需要异步调度的数据必须在回调内解码
  到明确拥有的 Request/参数对象；
- 不为统一 TCP/NATS 所有权而在热路径引入 `Packet` 接口、每消息 Release 闭包或引用计数；
- 若 M11 基准证明大字节字段复制成为主要瓶颈，再单独 Review“可转移所有权 Packet”方案，
  不在 M6 预先增加复杂度。
...
6. M11 可以根据 RPC 消息分类决定告警、停止 Subscription、让 Node 退休或停服。

M11 的 Handler 已经收到消息、但 Origin RPC 入口队列已满时：
...
- M11 Adapter 只做快速协议校验、必要所有权转换和调度投递；
- Handler 不能无限阻塞；
...
- M11 的 RPC 请求必须在线协议中携带 Deadline，远端拒绝已经过期的迟到请求。
...
- M7/M11 后续把事件转换为 Node/Service 调度事件；
- M6 不为事件建立第二个有界队列；
- EventHandler 必须快速返回；
...
8. 状态事件和日志不进入普通消息热路径；
9. 不使用 Headers 承载 Origin RPC 元数据，M11 统一放入 RPC payload 或 Subject；
10. Origin RPC 不调用原生同步 Request，避免与 M9 重复建立响应 Channel、pending 状态和
    Deadline；
11. M11 每个 Node 只使用稳定的请求/响应 Subscription，不按 Service 或 RPC 调用创建
    Subscription；
...
- 都不执行 Service 业务逻辑；
- 都限制消息大小和内存；
- 都由 M10/M11 Adapter 接入同一 RPC Runtime。
...
- M9 拥有生成客户端语义、RequestID、pendingCall、默认 `15s` Deadline、取消、统一错误码
  和响应完成；
- M10 TCP Adapter 只负责把统一 RPC 帧发送到选定 TCP Connection，并把入站帧交给 M9；
- M11 NATS Adapter 只负责把同一 RPC 帧发布到选定的稳定 Subject，并把请求/响应订阅收到
  的数据交给 M9；
...
- Transport 发送成功都只表示“本地传输层已接受”，RPC 完成必须等待 M9 收到响应或
  Deadline；
- M9 的同步入站处理函数在返回后不能继续引用传入的原始字节；TCP Adapter 随后归还
...
14. M6 不定义任何 Origin Subject，全部留给 M11 和服务发现系统；
15. 不建立 TCP/NATS 共同大接口，但 M10/M11 必须接入同一 M9 RPC Runtime 和统一线协议；
16. M11 不使用 NATS 原生 Request/Reply；每个 Node 使用稳定的 Node 级请求/响应 Subject
