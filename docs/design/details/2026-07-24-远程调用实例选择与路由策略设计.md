...
- 当前状态允许普通 RPC 路由；
- 未进入 `Stopping` 或 `Stopped`；`Retired` 与 `Running` 使用相同候选规则；
- 通过后续确定的健康状态检查。
...
`BroadcastXxx` 忽略这些单目标选择策略，只使用 Target 的基础 ServiceName/Node 范围。
...
15. 候选数量变化后按新数量重新取模；
16. 停止、未就绪和不健康实例不会进入 Selector；Retired 实例仍可进入 Selector；
