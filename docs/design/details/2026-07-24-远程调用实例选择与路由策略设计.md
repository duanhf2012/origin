...
`origingen` 同时生成最常用的便捷绑定入口：

```go
playerRPC := contract.BindPlayerRPC(s, "PlayerService")
```

它等价于上面的规范构造。客户端是可长期保存的轻量值，推荐在 Service 启动时绑定到字段，
后续普通调用只写 `s.playerRPC.AwaitXxx(...)`，不重复创建 Target 或设置策略。
...
`RouteRoundRobin`、`RouteRandom`、`Route` 和 `RouteBy` 都返回同一种生成强类型客户端。
它们只改变单目标选择策略，不改变 `AwaitXxx`、`AsyncXxx` 和 `NotifyXxx` 的签名；后续
Broadcast 里程碑单独定义多目标范围和部分失败语义，M19 不扩展 Broadcast。
...
必须保存实际 `NodeID + ServiceName`，并使用
`NewXxxRPCClient(owner, rpc.ToServiceOnNode(...))` 精确调用；迁移状态需要单独的业务协议。
...
```go
nodeRPC := contract.NewPlayerRPCClient(
    s,
    rpc.ToServiceOnNode(
        "player-2",
        "PlayerService",
...
- `Route(any)` 对基础类型和命名整数分别执行逃逸分析；若接口装箱产生稳定堆分配，实施
  必须增加无分配快速入口或调整内部签名。
...
30. `BindXxxRPC`、客户端值派生、Prepare 和基础 Key 路径的逃逸分析与零分配基准。

## 14. 已确认结论

- `ToService` 目标默认使用轮询；
- 提供 `BindXxxRPC(owner, serviceName)` 便捷绑定并支持作为 Service 字段长期复用；
