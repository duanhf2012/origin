...
`NewXxxRPCClient(owner, rpc.ToNodeService(...))` 精确调用；迁移状态需要单独的业务协议。
...
`ToNodeService` Target 的目标范围最多只有指定的 `NodeID + ServiceName` 一个实例：
...
因此，对 `ToNodeService` Target 调用 `RouteRoundRobin`、`RouteRandom` 或 `Route(key)` 不会
扩大目标范围，仍只能选择 `player-2 + PlayerService`。`RouteBy` 只会收到零个或一个
候选实例。

底层 TCP 重连、NATS 重订阅或快照对象替换都不能改变 `ToNodeService` Target 的逻辑目标。
