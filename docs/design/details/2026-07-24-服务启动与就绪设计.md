...
`OnInit` 只完成配置解析、Module 静态装配和不需要清理的内存状态初始化。任一 Service 的
`OnInit` 返回非 nil error 时：

1. 立即停止初始化当前 Node 的后续 Service；
2. 当前 Node 不调用任何 Service 的 `OnStart`，也不进入 `Ready`；
3. Node 使用 `%w` 或等价错误链保留原始 error，并补充 NodeID、ServiceName 和
   `on_init` 阶段；
4. Node 向 Application 返回启动失败；
5. Application 停止后续 Node，并严格反序停止此前已经启动成功的全部 Node；
6. 全部回滚结束后，`app.Start()` 打印一次 Error 日志并以非零退出码结束程序。

当前失败 Node 中只完成 `OnInit` 的 Service 不调用 `OnStop`。因此 `OnInit` 不得获得数据库
连接、启动 goroutine、Timer 或其他必须释放的外部资源；这类工作必须进入 `OnStart`。
Service 和 Node 不重复打印向上传递的同一个 error。
...
发布。任一 Module 启动失败都会按已经成功的启动顺序逆序回滚，并使 Node 启动失败。
