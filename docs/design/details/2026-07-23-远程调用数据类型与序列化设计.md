...
| 里程碑 | 实现范围 | 明确不实现 |
|---|---|---|
| M11 | 顶层 Protobuf、基础整数与浮点数、布尔值、字符串、`[]byte`；生成器类型识别、契约指纹、同 Node 编解码与 Dispatcher 闭环 | 普通结构体、指针、数组、Slice、Map、嵌套 Protobuf、自定义 Codec、TCP、NATS |
| M12 | 普通 Go 类型的完整静态编解码、稳定字段 ID、指针和容器、普通结构体、嵌套 Protobuf、自定义 Codec、兼容性检查 | TCP、NATS、RequestID、pendingCall、发现和路由 |
| M13 | 复用 M11/M12 数据表示接入 TCP RPC | 新增或修改业务数据编码规则 |
| M14 | 复用 M11/M12 数据表示接入 NATS RPC | 新增或修改业务数据编码规则 |
...
该布局只用于方法顶层位置。M12 普通结构体仍按稳定字段 ID 设计，不能把结构体字段退化为
...
M11 扫描到属于 M12 范围的合法类型时，应明确报告“当前里程碑尚未实现”，并展示完整
方法和参数路径；这属于阶段性能力边界，不得误报为永久禁止。M12 完成后，相同声明必须
按本文最终规则生成。对于接口、函数、Channel、循环对象图等最终不支持的类型，则继续按
永久不支持错误处理。
