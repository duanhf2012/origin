...
该布局只用于方法顶层位置。M11 普通结构体仍按稳定字段 ID 设计，不能把结构体字段退化为
依赖声明顺序的无 Tag 布局。需要 Protobuf 字段级演进能力时，业务应使用一个顶层
Protobuf 请求或响应；外部 gRPC 插件负责完整协议适配，不要求与 Origin Native RPC 共用
方法载荷格式。
...
## 12. M11 普通 Go 类型尚需确认的实现细节

普通 Go 类型已经前移到 M11。以下内容不是功能遗漏，而是 M11 编码前必须继续锁定的
实现细节：

1. 稳定字段 ID 如何生成和持久化，如何处理字段重命名、删除和禁止编号复用；
2. Map 是否要求跨进程输出确定性字节，以及为此排序带来的 CPU 和分配成本；
3. M12 自定义静态 Codec 的最小接口、优先级、错误语义和 Buffer 所有权；
4. 递归类型与循环引用的静态拒绝规则；
5. 解码长度、元素数量和嵌套深度的安全上限；
6. Schema 兼容性检查与完整契约指纹如何组合；
7. 普通结构体路径下 Protobuf Open/Hybrid 生成代码在不同 API Level 的测试样本。

除自定义静态 Codec 外，这些细节必须在 M11 设计 Review 中确认后再编码，也不能延后到
TCP/NATS Transport 中临时决定。自定义静态 Codec 单独进入 M12，不阻塞 M11 对内置支持
类型的静态编解码。
