...
“优先复用”不表示禁止扩展。如果后续实现或 Provider 出现需要调用方稳定区分、会跨进程
传递且现有 Code 无法准确表达的发现领域错误，必须新增专用 Code，并在本文记录触发条件
和恢复语义；不能为减少枚举而滥用 `CodeInternal` 或 `CodeInvalidConfig`。M14 当前不预占
服务发现编号段。
