...
- Service 查询；
- RPC 候选路由构建；快照保留 Running/Retired，M19 的 `ToService` 自动选择只取
  Running，精确目标仍允许 Retired；
...
状态拒绝入站或移出精确路由；M19 自动选择会排除 Retired，业务仍可在精确调用的目标
Service 内根据状态自行处理。
...
Service 一定会接受业务请求。Retired 继续可见且可精确调用，但被 M19 自动选择排除。
...
28. `Retired` 只作为可观察状态发布，框架不自动拒绝精确 RPC/Notify；M19 自动
    `ToService` 排除 Retired，Broadcast 由后续里程碑确定；
