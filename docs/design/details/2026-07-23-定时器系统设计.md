...
Timer 回调最终如何与其他 Service 任务交错，由后续 Service 执行模型设计决定。本文只规定 TimerEngine 不直接执行用户代码，以及同一个周期 Timer 不得与自己重叠。Timer 内部对象的池化、Reset 和防 ABA 规则见 [Origin v3 内存复用与对象池设计](./2026-07-25-内存复用与对象池设计.md)。
...
TimerEngine 从首版实现开始使用内部 `timerEntry` 专用池，但不池化或手工复用 Go 标准库的 `time.Timer`。只有确认 Timer 已经脱离时间轮、TimerID 索引、暂停列表、待投递列表和运行中回调后，才能清理并放回池。

每次复用 `timerEntry` 都分配新的单调递增 TimerID。TimerID 不使用对象地址或池索引，旧 ID 不能操作已经复用的对象。释放前必须清空 callback、Context、Service、Module、所有者和时间轮节点引用，完整规则见 [Origin v3 内存复用与对象池设计](./2026-07-25-内存复用与对象池设计.md)。
...
- TimerEngine 不执行用户回调；
- TimerEngine 从首版开始使用内部 `timerEntry` 专用池；
- 每次复用分配新的 TimerID，只有全部引用退出后才 Reset 并回池；
