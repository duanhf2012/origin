...
`BindXxxRPC(owner, serviceName)` 是最常用 `ToService` 的便捷入口，等价于调用规范构造
函数，不引入第二套路由或客户端实现。
...
一个客户端类型加一个规范构造函数可以避免为每个契约重复生成自动目标、Node 目标及未来
路由目标的多个构造函数。`BindXxxRPC` 只缩短最常见自动目标的初始化代码；精确目标和其他
Target 仍通过 `NewXxxRPCClient`、`ToService` 与 `ToServiceOnNode` 清楚表达语义，也不需要
用空 NodeID、可变参数或隐含 Options 表示不同模式。
...
func (s *PlayerService) OnInit() error {
    s.dbClient = contract.BindDBRPC(s, "DBService")
...
构造、`BindXxxRPC`、内置策略派生和成功 Prepare 热路径必须通过逃逸分析与 Benchmark
证明不产生框架堆分配。
...
- `BindXxxRPC`、内置策略派生和成功 Prepare 是零框架堆分配硬门禁；
...
- `BindXxxRPC(owner, serviceName)` 是等价于 `ToService` 的便捷绑定入口，推荐作为 Service
  字段长期复用；
