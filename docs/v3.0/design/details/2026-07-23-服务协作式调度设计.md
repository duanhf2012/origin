...
Service 处于 `Stopping/Draining` 时，Stop 前已接收任务和它们的框架回调仍使用上述普通
Await 路径，可以继续进入 Waiting、创建替代 Runner 并受控恢复。Service 仍然只有一个
执行槽，不会有其他普通业务任务并发进入；新的入站 RPC、Timer、普通本地事件和根任务均
拒绝。业务可以创建自己持有、可以取消并可以等待退出的 goroutine 或固定工作池，但它们
没有有效 Task Context，不能调用 Await、创建属于 Service 的 Timer 或直接并发访问 Service
可变状态。跨 goroutine 计算结果必须通过线程安全的 `DispatchAsync` 切回 Service 唯一
执行槽；进入 Draining 后新 Dispatch 返回 `CodeServiceStopping`。框架不拦截 Go 的
`go` 语句，只在自己的 API 边界校验 Context 和生命周期。由于所有需要 Service 执行权的
用户代码仍只在活动 Runner 中执行，不需要为排空延续创建 `TaskScope` 或父子任务树。
...
继续使用调用方已有的 Go Runtime Timer，不再登记 M8；没有 Deadline 的合法 Context
使用统一默认值和一条 M8 Deadline。M8 派生 Context 必须公开计算后的 `Deadline()`，
且不创建第二个 Go Runtime Timer。

`context.Background()`、`context.TODO()` 在超时选择层都属于“没有 Deadline”，但
`Service.Await` 仍要求外层 Context 携带当前 Origin Task 执行权令牌。业务应传入框架回调
提供的 Task Context；该 Context 没有显式 Deadline 时自然使用 M8 默认超时，不能用裸
Background 伪造当前任务。
...
- `OnStop` 由最后一个 Service Runner、空闲等待 Runner 或极端回滚路径中的唯一专用
  finalizer Runner 执行，Node Stop 调用方不直接执行 Service 业务代码；
