...
| 里程碑 | 目标 |
|---|---|
| M0：最小闭环 | 两个 Node、两个 Service 通过静态地址和 TCP 完成生成式 Protobuf Await RPC、调度交错及优雅停止 |
| M1：核心完备 | 补齐 Module、发现、退休、Timer、事件、完整 RPC 语义、路由、序列化、NATS、etcd 和生产级可观测性 |
...
| Service 注册与生命周期 | `service/service.go`、`service/manager.go` | M0 先实现基础状态、Await 排空和 OnStop；就绪门、退休后置 | M0/M1 | 启停、排空、无停止后回调 |
| Module 生命周期 | `service/module.go` | 保留并改为静态树，生命周期与 Service 对齐 | M1 | 父子启动顺序和逆序释放 |
| Node 内 Service 启停顺序 | `node/`、配置示例 | 保留显式顺序；未配置项按声明顺序补齐，停止默认反序 | M1 | 部分配置、重复项、未知项校验 |
...
| 本地异步事件 | `event/` | 保留 | M1 | `NotifyAsync` 下一任务触发 |
| 本地同步事件 | `event/` | 新增明确的 `NotifySync` | M1 | 同一调用栈、禁止跨 Service 并发 |
| 内部 RPC/Await Deadline | `util/timer/`、`rpc/` | M0 只提供内部一次性 Deadline 边界 | M0 | 超时、取消和资源释放 |
| 业务 Timer 与 Cron | `util/timer/`、Service 定时接口 | 重构为 Node 统一时间引擎和 Service 投递 | M1 | 取消、暂停、积压、停止清理 |
...
| 异步 RPC | `rpc/` | 保留为生成的 `AsyncXxx` | M1 | 完成结果回到目标 Service 调度器 |
| 可取消异步 RPC | v2 `withcancel` 风格 | 保留取消能力，规范化句柄和 Context | M1 | 取消竞态与迟到响应 |
| 无返回通知 | v2 `Go` 风格 | 重构为生成的 `NotifyXxx` | M1 | 只保证接受结果，不伪装业务成功 |
...
| 任意实例调用 | `rpc/` | 保留并显式路由策略 | M1 | RoundRobin、Rand、ModKey、自定义路由 |
| 本地 RPC 优化 | v2 本地调用路径 | M0 不做透明短路，先保持网络行为一致 | M1 | 短路前后序列化、超时和错误一致 |
| TCP RPC | `rpc/server.go`、`rpc/client.go` 等 | M0 实现静态目标、帧、pending、断线失败和有界队列；重连后置 | M0/M1 | 真实网络、短写、断线、背压、包大小限制 |
...
| 最大包限制与压缩 | `rpc/compress.go`、配置 | 最大包限制进入 M0，压缩后置 | M0/M1 | 恶意包、内存上限和压缩阈值 |
...
| v2 能力 | v2 参考位置 | v3 处理 | 里程碑 | 验证重点 |
|---|---|---|---|---|
| 内置服务发现 | `cluster/origin_discovery.go` | 保留，不依赖外部中间件 | M1 | 注册、更新、失去发现、退休状态 |
| etcd 服务发现 | `cluster/etcd_discovery.go` | 保留为可插拔实现 | M1 | 租约、断线恢复、事件去重 |
| TTL 与失效处理 | `cluster/ttl.go` | 保留并统一到发现状态机 | M1 | 时钟漂移和假死实例清理 |
| 发现与失去发现事件 | `cluster/` | 保留，并增加退休状态事件 | M1 | 顺序、重复事件和状态快照 |
| 私有 Node/Service | `cluster/` 与配置 | 用关注规则和发布范围重构 | M1 | 不应暴露的实例不会建立连接 |
| 正则发现筛选 | v2 `AllowDiscovery` | 替换为契约、Node 名、Service 名和多值标签规则 | M1 | 先筛选后建连、规则可组合 |
| Global/Service/NodeService 配置 | v2 JSON/YAML 配置 | M0 只解析 Node、静态地址和运行时限制；完整层级后置 | M0/M1 | 最小启动配置与后续覆盖优先级 |
| JSON/YAML 与环境变量 | v2 配置加载 | YAML 作为 M0 主路径；其他格式和展开后置 | M0/M1 | 明确错误位置 |
| 读取字段或解析结构体 | v2 Service 配置接口 | 保留两种方式并规范化 | M1 | Module 可通过 Service 获取配置 |
...
| v2 能力 | v2 参考位置 | v3 处理 | 里程碑 |
|---|---|---|---|
| 日志 | `log/` | M0 只使用最小日志能力，正式接口和实现后置 | M0/M1 |
| 系统信号与进程控制 | `node/platform_*.go`、`util/sysprocess/` | 示例可把停止信号转为 Context 取消，完整系统后置 | M0/M1 |
| 控制台命令 | `console/` | 后置 | M3 |
...
1. M0 只保留 Application、Node、Service、Await、静态 TCP RPC、最小配置和优雅停止的核心闭环。
2. Module、发现、退休、业务 Timer、事件和完整异步 RPC 语义在 M1 补齐。
3. 接口命名可以规范化，但尽量维持 v2 用户熟悉的调用结构。
4. 同进程多 Node 仍通过 RPC 传输边界交互，不引入只在开发环境成立的隐式行为。
5. NATS、etcd、外部 gRPC 和 v2 的系统 Module 不阻塞 M0。
6. 每补齐一个后续能力，都必须同时补兼容说明、测试和性能基线。

## 12. 盘点结论

v2 的能力面明显大于 v3 首个闭环。最稳妥的行动顺序是：

1. 用 M0 验证生命周期、调度、静态 TCP RPC、内存复用和停止流程能够真正协同工作。
2. 再补齐 Module、发现、退休、Timer、事件、NATS、etcd、完整路由和序列化等核心能力。
